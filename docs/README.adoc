...
After creation of an `Istio` resource, the Sail Operator will generate a revision name for it based on the updateStrategy that was chosen, and create a corresponding link:#istiorevision-resource[`IstioRevision`].
...
NOTE: The CNI plugin at version `1.x` is compatible with `Istio` at version `1.x-1`, `1.x` and `1.x+1`.
...
==== Conditions
All resources have a `Ready` condition which is set to `true` as soon as all child resource have been created and are deemed Ready by their respective controllers. To see additional conditions for each of the resources, check the link:api-reference/sailoperator.io.md[API reference documentation].
...
== API Reference documentation
The Sail Operator API reference documentation can be found link:api-reference/sailoperator.io.md[here].