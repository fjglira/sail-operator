...
:istio_latest_version: 1.28.3
:istio_latest_version_revision_format: 1-28-3
:istio_latest_tag: v1.28-latest
:istio_release_name: release-1.28
:istio_latest_minus_one_version: 1.28.2
:istio_latest_minus_one_version_revision_format: 1-28-2
...
// test-start: Istiod HA ReplicaCount
...
// test-step-start: Create namespace and apply Istio HA replicaCount

[source,go-e2e-step,step-name="Create namespace and apply Istio HA replicaCount"]
----
...
// test-step-start: Verify istiod HA pods (replicaCount)

[source,go-e2e-step,step-name="Verify istiod HA pods (replicaCount)",retry=3,retry-interval=5s]
----
...
// test-start: Istiod HA Autoscaling
...
// test-step-start: Create namespace and apply Istio HA autoscaling

[source,go-e2e-step,step-name="Create namespace and apply Istio HA autoscaling"]
----
kubectl create ns istio-system
cat <<EOF | kubectl apply -f-
apiVersion: sailoperator.io/v1
...
// test-step-start: Verify istiod HA pods (autoscaling)

[source,go-e2e-step,step-name="Verify istiod HA pods (autoscaling)",retry=3,retry-interval=5s]
----
kubectl get pods -n istio-system -l app=istiod
----
