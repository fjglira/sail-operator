...
1. Enable the multi root support:
    ...
    [NOTE]
    ====
    Visit Istio documentation for details about `ISTIO_MULTIROOT_MESH` and `PROXY_CONFIG_XDS_AGENT`.
    ====
    ...
    ----
1. Prepare new root and intermediate certificates (you should be using trusted root CA for issuing the intermediate certificate). Here we are using link:https://github.com/istio/istio/tree/master/tools/certs[tooling] from the istio repository:
    [source,bash]
    ----
    mkdir -p certs
    pushd certs
...
1. Create new `cacerts` secrets with old CA certificate, key and chain and new combined root certificates:
    [NOTE]
    ====
    It's necessary to assure that all workloads trust both old and new root certificates before updating the certificate used for signing workload certificates to avoid traffic disruptions.
    ====
...
1. Verify that all workloads are trusting both old and new roots, e.g. for httpbin:
    ...
    [NOTE]
    ====
    It might be necessary to restart the workload if you only see one certificate.
    ====
