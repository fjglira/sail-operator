...
- link:#documentation-guidelines[Documentation Guidelines]
  - link:#introduction[Introduction]
  - link:#documentation-structure[Documentation Structure]
    - link:#structure-of-the-entire-folder-and-new-folders[Structure of the entire folder and new folders]
  ...
  - link:#automated-testing-over-the-documentation[Automated Testing over the Documentation]
    - link:#what-does-update-docs-examplessh-do[What does update-docs-examples.sh do?]
    - link:#adding-a-topic-example-to-the-automation-workflow[Adding a topic example to the automation workflow]
    - link:#debugging-a-specific-docs-example-locally[Debugging a specific docs example locally]
...
== Automated Testing over the Documentation
Any documentation step need to be tested to ensure that the steps are correct and the user can follow them without any issue. To do this, we use `runme` (check the docs link:https://docs.runme.dev/getting-started/cli[here]) to run the tests over the documentation. The workflow of the automation is the following:
...
- Check all the md files in the `docs` folder and exclude from the list the files described inside the `EXCLUDE_FILES` variable and then copy all the files to the `tests/documentation_test/` folder that meets the criteria, check link:#automated-testing-over-the-documentation[Adding a topic example to the automation workflow] section for more information about the criteria.
- Once the files are copied into the destination path it checks those files to uncomment the bash commented steps. This bash commented code block are going to be hiden in the original md files but we will uncomment this code block to be able to run validation steps that wait for conditions an avoid flakiness of the test. This means that most of the validation steps will be commented code blocks, more information link:#adding-a-topic-example-to-the-automation-workflow[here].
...
-->
....
To check the entire list of prebuilt functions please check the link:../../tests/documentation_tests/scripts/prebuilt-func.sh[prebuilts-func.sh] script. Note that `SCRIPT_DIR` is a variable that is already defined in the `run-docs-examples.sh` script, so you can use it directly in your documentation.
